...
const UpdateTask = ({ taskId, setTasks, clearSelectedTask }) => {
  ...
  useEffect(() => {
    const fetchTaskDetails = async () => {
      try {
        ...
        const { taskname, date, time, notistatus } = response.data[0];
        setTaskName(taskname);
        setTaskDate(formatDate(date));
        setTaskTime(time);
        setNotiStatus(notistatus);
  ...
  const startListeningForField = () => {
    if (keyboardClicked || !("webkitSpeechRecognition" in window)) return;
    speak(
      "Which field do you want to change? Task name, Task date, or Task time?",
      startListeningForFieldRecognition
    );
  };
  ...
  const extractTimeComponents = (transcript) => {
    ...
    if (match) {
      ...
      const minutes = match[2];
      let meridiem = match[3] || "";
      if (meridiem) meridiem = meridiem.toLowerCase();
      return { hours, minutes, meridiem };
    } else {
      return { hours: "00", minutes: "00", meridiem: "" };
    }
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            type={keyboardClicked ? "date" : "text"}
            id="taskdate"
            value={taskDate}
        ...
        <div>
          ...
          <input
            ...
            type={keyboardClicked ? "time" : "text"}
            id="tasktime"
            value={taskTime}
